...
se1_unit_test.out: $(TEST_OBJ) $(OBJ)
	$(CXX) -o se1_unit_test.out $(LDFLAGS) $(filter-out src/main.o, $(OBJ)) $(filter-out test/fsm_test_main.o, $(TEST_OBJ)) $(LDLIBS)

# link everything together for fsm test executable
se1_fsm_test.out: $(TEST_OBJ) $(OBJ)
	$(CXX) -o se1_fsm_test.out $(LDFLAGS) $(filter-out src/main.o, $(OBJ)) $(filter-out test/unit_test_main.o, $(TEST_OBJ)) $(LDLIBS)
